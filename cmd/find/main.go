...
func init() {
	...
	flag.StringVar(&entityType, "eType", "", "Entity type to search for")
	flag.StringVar(&fields, "fields", "", "Comma-separated list of fields to display")
	flag.Parse()
}
...
func main() {
	...
	if query == "" {
		qlog.Error("No condition provided")
		os.Exit(1)
	}

	store := qstore.New()

	app := qapp.NewApplication("qfind")
	...
	oneShotWorker.Connected().Connect(func(ctx context.Context) {
		...
		start := time.Now()
		entities, err := store.Find(ctx, qdata.EntityType(entityType), qdata.CastSlice(strings.Split(fields, ","), func(s string) qdata.FieldType {
			return qdata.FieldType(s)
		}), query)
		if err != nil {
		...
		results.Headers = strings.Split(fields, ",")
		for _, entity := range entities {
			row := qdata.NewQueryRow()
			for _, field := range results.Headers {
				row.Set(field, entity.Field(qdata.FieldType(field)).Value, true)
			}
